...
$app->get('/ticket/new', function (Request $request, Response $response) {
    ...
    $response = $this->view->render($response, "ticketadd.phtml", ["components" => $components, "router" => $this->router]);
    return $response;
})->setName('ticket-new');
...
$app->post('/ticket/new', function (Request $request, Response $response) {
    ...
    $response = $response->withRedirect($this->router->pathFor('tickets'));
    return $response;
});
...
$app->get('/ticket/{id}', function (Request $request, Response $response, $args) {
    ...
    $response = $this->view->render($response, "ticketdetail.phtml", ["ticket" => $ticket,"router" => $this->router]);
    return $response;
...
$app->delete('/ticket/{id}', function (Request $request, Response $response, $args) {
    $ticket_id = (int)$args['id'];
    $mapper = new TicketMapper($this->db);
    $ticket = $mapper->delete($ticket_id);
    $response = $response->withRedirect($this->router->pathFor('tickets'));
    return $response;
